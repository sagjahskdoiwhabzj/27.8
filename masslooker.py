...
async def add_channel_to_queue(username: str, user_id: int = None):
    ...
    # ИСПРАВЛЕНИЕ: объявляем global переменные в самом начале функции
    global shared_client, settings
    
    if not user_id:
        # Если user_id не передан, используем глобальную логику как в оригинале
    ...
    # Используем реальную логику подготовки канала для конкретного пользователя
    if user_id not in user_channel_processing_queue:
        user_channel_processing_queue[user_id] = {}
    
    # Инициализируем user_processed_channels если не существует
    if user_id not in user_processed_channels:
        user_processed_channels[user_id] = set()
    ...
    # Используем существующую функцию подготовки канала
